  ...
  return (
    <div className="space-y-6 p-1">
      <div>
        ...
        <p className="text-muted-foreground text-sm mt-0.5">Monitor all tutoring sessions across the platform.</p>
      </div>

      {/* Stats bar */}
      <div className="grid gap-3 grid-cols-2 sm:grid-cols-4">
        {Object.entries(stats).map(([label, count]) => (
          <div key={label} className="border rounded-2xl px-4 py-4 text-center bg-card hover:shadow-sm transition-shadow">
            <p className="text-xs text-muted-foreground capitalize font-medium">{label}</p>
      ...
      <div className="border rounded-2xl overflow-x-auto bg-card shadow-sm">
        <table className="w-full text-sm">
          <thead className="bg-muted/60 text-muted-foreground border-b">
            <tr>
              <th className="px-4 py-3 text-left font-semibold text-xs uppercase tracking-wide whitespace-nowrap">Student</th>
